...
cpu6502::cpu6502()
{
	// Lookup table for the disassember (prints OPCODE), a giant initialiser list of initialiser lists...
	//These are indexed by OPCODE so all you do is lookup[opcode].name etc to get members
	lookup =
	{
...
{
	//pipe the bus read output to the cpu's output, readonly bit is false
	return bus->read(a, false);
}

void cpu6502::write(uint16_t a, uint8_t d)
{
	//pipe the cpu's write input to the bus's input
	bus->write(a, d);
}
...
void cpu6502::clock()
{
	if (cycles == 0)
	{
		opcode = read(pc); //reads current byte
		pc++; //goes to next byte

		// Get Starting number of cycles
		cycles = lookup[opcode].cycles;

		//actually a function call to execute the op code (just using the function pointer stored in lookup)
		uint8_t additional_cycleA = (this->*lookup[opcode].addrmode)();
		uint8_t additional_cycleO = (this->*lookup[opcode].operate)();

		//if both the addressmode and opcode request an additional clock cycle increment it
		cycles += (additional_cycleA & additional_cycleO);
	}

	//one cycle has elapsed everytime we call the clock
	cycles--;
}
...
///////////////////////////////////////////////////////////////////////////////////////////////////////
//ADDRESS Modes (a=accumulator,x=x reg,y=y reg,stkp=stack pointer,pc=program counter,status=status reg)
///////////////////////////////////////////////////////////////////////////////////////////////////////
...
// one byte instead of the usual two. This is why most working memory is located around page zero
uint8_t cpu6502::ZP0()
{
	addr_abs = read(pc);
	pc++;
...
// ranges within the first page, kind of like using an array in C
uint8_t cpu6502::ZPX()
{
	addr_abs = (read(pc) + x);
	pc++;
...
///////////////////////////////////////////////////////////////////////////////////////////////////////
//OPCODE functions (a=accumulator,x=x reg,y=y reg,stkp=stack pointer,pc=program counter,status=status reg)
///////////////////////////////////////////////////////////////////////////////////////////////////////
...
uint8_t cpu6502::ADC()
{
	...
	// We use 16-bit int instead of 8-bit to capture any carry bit, which will exist at bit 8
